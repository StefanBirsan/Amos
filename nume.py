...
async def on_raw_reaction_add(reaction):
     if reaction.message_id == 1102630914799976498:
...
async def verify(ctx):
    msg = await ctx.send("Verify za homies!")
    emoji = "👍"
...
async def setautorole(ctx, role: discord.Role):
    with open("/pateu/autorole.json", "r") as f:
        auto_role = json.load(f)

        auto_role[str(ctx.guild.id)] = role.name

    with open("/pateu/autorole.json", "w") as f:
...
@client.event
async def on_guild_join(guild):
    with open("/pateu/mute.json", "r") as f:
        mute_role = json.load(f)

        mute_role[str(guild.id)] = None

    with open("/pateu/mute.json", "w") as f:
...
@client.command(description = 'roll a dice')
async def roll(ctx, choice=None):
    if choice == None or choice != 'd20' and choice != 'd6' and choice != 'd4' and choice != 'd8' and choice != 'd10' and choice != 'd12':
        await ctx.send("Please enter a valid dice!")
    if choice == 'd20':
        number = random.randint(1,20)
        await ctx.send(f"You just rolled a fantastic {number}")
...
async def help(ctx):
    ...
    help_embed.add_field(name="math", value="Too lazy to do some simple math? Use this command.", inline= False)
    help_embed.add_field(name="roll", value="Roll any dice between d4 to d20.", inline= False)
    help_embed.add_field(name="version", value="Show's the bot's version and details.", inline= False)
    help_embed.add_field(name="userinfo", value="Show's the user's info ofc.", inline= False)
    help_embed.add_field(name="svinfo", value="Show's the server's info ofc.", inline= False)
    help_embed.add_field(name="meme", value="I wonder this does.. hmm.., use *meme and find out", inline=False)
    help_embed.add_field(name="cat", value="If the reddit gods like you you might get a cat", inline=False)
    help_embed.add_field(name="emojify" , value="Turn a picture into emojis , you have to use the link of that pic" , inline=False)
    ...
    help_embed.add_field(name="work", value="You can work for some money.", inline= False)
    help_embed.add_field(name="deposit", value="Deposit your money at AMOS bank.", inline= False)
    help_embed.add_field(name="withdrawl", value="Withdraw your money from the bank.", inline= False)
    help_embed.add_field(name="steal", value="Steal some money from a user.", inline= False)
    help_embed.add_field(name="coinflip", value="Win some money by betting on a toss of coin.", inline= False)
    help_embed.add_field(name="a_help", value="This is only for the mods.", inline= False)
...
async def a_help(ctx):
    ...
    a_help_embed = discod.Embed(title = "Help desk for administrators", description= "All commands in one place", color=0x5F57A9)
    
    a_help_embed.add_field(name="purge", value="Kills an input number of messages :D ", inline=False)
    a_help_embed.add_field(name="kick", value="I wonder this does.. hmm..", inline=False)
    a_help_embed.add_field(name="ban", value="I guess you know what this does right?", inline=False)
    a_help_embed.add_field(name="unban" , value="Unban yo homie" , inline=False)
    a_help_embed.add_field(name="mute", value="Shows you how much money you got.", inline= False)
    a_help_embed.add_field(name="unmute", value="Make some money by begging.", inline= False)
    a_help_embed.add_field(name="setmuterole", value="Show's the user's info ofc.", inline= False)
    a_help_embed.add_field(name="setautorole", value="Show's the user's info ofc.", inline= False)
    a_help_embed.set_footer(text=f"Requested by @{ctx.author}.", icon_url=ctx.author.avatar)
...
async def balance(ctx, member: discord.Member=None):
    with open("/pateu/eco.json", "r") as f:
    ...
    if str(member.id) not in user_eco:
        ...
        with open("/pateu/eco.json", "w") as f:
